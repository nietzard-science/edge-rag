...
> are larger) — they are distributed separately. Download the artifact bundle
> and unpack it into `data/<dataset>/` so the paths above resolve:
>
> > **Artifact bundle:** `<ARTIFACT_BUNDLE_URL>`
> > *(TODO — replace with the public download link before submission. Zenodo is
> > recommended: free, versioned, and gives a citable DOI for the artifact.)*
>
...
deselected). The 738-test full collection covers the data layer, logic
...
# Expected: 720 passed, 18 deselected (the nightly subset).
