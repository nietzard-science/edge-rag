...
class AgentPipeline:
    ...
    def process(self, query: str) -> PipelineResult:
        ...
        # Never cache an error result: a transient infrastructure failure
        # (Ollama hiccup, timeout) must not become a sticky instant-error for
        # this query for the rest of the process lifetime — fatal in an
        # interactive/demo session where the natural reaction is to retry.
        if confidence_val != "error":
