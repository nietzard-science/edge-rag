...
# Project root (this file -> data_layer -> src -> root). The default embedding
# cache is anchored here rather than to the current working directory, so a
# script launched from a subdirectory does not spray a stray ./cache/ tree.
# settings.yaml's embeddings.cache_path overrides this in production.
_PROJECT_ROOT: Path = Path(__file__).resolve().parent.parent.parent
_DEFAULT_CACHE_PATH: Path = _PROJECT_ROOT / "cache" / "embeddings.db"
...
class EmbeddingCache:
    ...
    def get(self, text: str, model_name: str) -> Optional[List[float]]:
        ...
        try:
            ...
            # Non-fatal by design: the caller re-embeds via the live API on a
            # cache miss (see docstring), so a corrupted/locked cache file
            # degrades to "always miss" rather than failing the query. Logged
            # at warning (not error) so a stale/corrupted cache — expected
            # after an interrupted process — doesn't read as a live failure
            # on every single lookup during a demo.
            logger.warning("Cache GET failed (falling back to live embed): %s", e)
            return None
    ...
    def put(self, text: str, embedding: List[float], model_name: str) -> None:
        ...
        try:
            ...
            # Non-fatal: the embedding the caller just computed is still
            # returned and used; only the cache write is lost, so the next
            # lookup simply re-embeds. See the GET-path note above.
            logger.warning("Cache PUT failed (embedding not cached): %s", e)
    ...
    def get_batch(
        ...
        try:
            ...
            # Non-fatal: an empty dict means every text below is treated as a
            # miss and re-embedded live. See the single-GET note above.
            logger.warning("Cache batch GET failed (falling back to live "
                          "embed): %s", e)
            return {}
...
class BatchedOllamaEmbeddings(Embeddings):
    ...
    # Ollama model keep-alive sent with every embed request. "30m" keeps the
    # embedding model resident between queries so an interactive/demo session
    # does not pay the 10–35 s cold reload after Ollama's ~5 min default
    # idle-unload. keep_alive affects model residency only, never outputs.
    ...
    def __init__(
        ...
        Args:
            ...
            keep_alive:  Ollama keep_alive for the embedding model (e.g. "30m").
                         Residency only — has no effect on embedding values.
    ...
    # Floor for the probe timeout: the first embed call after Ollama idles out
    # its model pays the full model (re)load — measured 10–35 s for
    # nomic-embed-text on the reference hardware — so the probe must tolerate a
    # cold load even when llm.timeout is configured short.
    _PROBE_TIMEOUT_FLOOR_S = 45

    def _test_connection(self) -> None:
        """
        Verify Ollama API availability and model presence.

        The probe doubles as the model warm-up: Ollama loads the embedding
        model on the first embed request, which can take 10–35 s cold. The
        probe therefore uses max(timeout, _PROBE_TIMEOUT_FLOOR_S) and retries
        once on a read timeout (with a visible log line) before giving up —
        a cold model must surface as a slow first call, not a crash.

        Raises:
            ConnectionError: API unreachable, or timed out on both attempts.
            RuntimeError:    Model not found (HTTP 404).
        """
        connection_timeout = max(self.timeout, self._PROBE_TIMEOUT_FLOOR_S)
        attempts = 2
        for attempt in range(1, attempts + 1):
            try:
                ...
                # Refused/unreachable — retrying cannot help; fail fast.
                raise ConnectionError(
                ...
                if attempt < attempts:
                    logger.warning(
                        "Ollama embed probe timed out after %ds — the embedding "
                        "model is likely cold-loading; retrying once (up to %ds)...",
                        connection_timeout, connection_timeout,
                    )
                    continue
                raise ConnectionError(
                    f"Ollama did not answer an embed request within "
                    f"{connection_timeout}s on {attempts} attempts. The model "
                    f"may still be loading — try again, or pre-warm it with: "
...
def create_embeddings(
    ...
    Parameter mapping from settings.yaml:
        ...
        embeddings.keep_alive     -> keep_alive (optional; default "30m")
    ...
    emb = BatchedOllamaEmbeddings(
        ...
        cache_path=Path(emb_cfg.get("cache_path", _DEFAULT_CACHE_PATH)),
        device=perf_cfg.get("device", "cpu"),
        timeout=llm_cfg.get("timeout", _D.DEFAULT_TIMEOUT),
        keep_alive=emb_cfg.get("keep_alive", _D.DEFAULT_KEEP_ALIVE),
    )
