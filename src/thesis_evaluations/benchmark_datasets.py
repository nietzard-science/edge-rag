...
def compute_exact_match(prediction: str, gold: str) -> bool:
    """HotpotQA-normalised EM with two documented relaxations (rules 2-3).

    NOT the official strict EM: the official metric is string equality after
    normalisation (rule 1 only). Rules 2 and 3 relax it for verbose SLM
    output; every system in the paper (Edge-RAG AND the BM25 baseline) is
    scored with this same function, so cross-system deltas are consistent.
