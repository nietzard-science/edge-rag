...
#    BOTH lines are required: lancedb/pylance are installed --no-deps to
#    sidestep a stale pyarrow<15 metadata pin (see requirements_frozen_nodeps.txt).
...
```bash
# 1. Fetch the pre-built stores into ./data (Zenodo artifact,
#    DOI 10.5281/zenodo.20807936 — the URL is pinned inside the script).
#    Unpacks data/<dataset>/{vector,graph,questions.json}; docker-compose
#    mounts ./data as the container's store root.
...
`CONFIG_PATH` (settings YAML), `INDEX_DIR` (store base; `DATASET_DIR` is an
accepted fallback alias) — so the container needs no code edits to relocate
Ollama or the mounted stores.
...
test_system/           Test suite (739 tests; 721 in the default CI selection)
