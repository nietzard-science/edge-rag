...
def build_pipeline(cfg: dict, dataset: str = _DEFAULT_DATASET):
    ...
    # Honour rag.retrieval_mode so the graph lane can be toggled on/off from
    # the demo (vector = dense+BM25, no graph; hybrid = all three lanes).
    _mode = str(rag_cfg.get("retrieval_mode", "hybrid")).lower()
    try:
        mode = RetrievalMode(_mode)
    except ValueError:
